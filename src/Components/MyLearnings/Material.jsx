      ...
      <div className="container">
        ...
        <div className="tab">
              ...
              {materials.length > 0 ? (
                paginatedMaterial.map((material, index) => (
                  <tr className="odd" key={material.id}>
                   <td>{(currentPageM - 1) * 10 + index + 1}</td>
                    <td>{material.type || '-'}</td>
                    <td>{material.name}</td>
          ...
          <div className="pagination" style={{ display: 'flex', justifyContent: 'flex-end', alignItems: 'center', margin: '15px 0 0px 0' }}>
              <button
                onClick={() => setCurrentPageM((prev) => Math.max(prev - 1, 1))}
                style={{ padding: '8px 15px', border: 'none', borderRadius: '5px', backgroundColor: '#3282c4', color: 'white', cursor: 'pointer' }}
                disabled={currentPageM=== 1}
              >
                Prev
              </button>
              <span style={{ margin: '0 10px' }}>Page {currentPageM} of {pages}</span>
              <button
                onClick={() => setCurrentPageM((prev) => Math.min(prev + 1, pages))}
                style={{ padding: '8px 15px', border: 'none', borderRadius: '5px', backgroundColor: '#3282c4', color: 'white', cursor: 'pointer' }}
                disabled={currentPageM=== pages}
              >
                Next
              </button>
            </div>
